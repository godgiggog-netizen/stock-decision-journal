...
function mountRadar(){
  ...
  button.textContent='Radar Alerts';
  ...
  createRoot(host).render(<RadarAlerts/>);

  const originalChildren=[...main.children].filter((el)=>el!==host);
  let active=false;
  const setRadar=(next)=>{
    active=next;
    originalChildren.forEach((el)=>{el.style.display=active?'none':'';});
    host.style.display=active?'block':'none';
    button.classList.toggle('active',active);
    [...nav.querySelectorAll('button')].filter((b)=>b!==button).forEach((b)=>{
