...
const mapPosition = (row) => ({
  ...
  lastReview: row.reviews?.[0]?.review_date || null,
  nextReview: row.next_review_date,
  thesisId: row.theses?.[0]?.id || null,
  thesis: row.theses?.[0]?.thesis_versions?.[0]?.thesis_text || '',
  expected: row.theses?.[0]?.thesis_versions?.[0]?.expected_outcome || '',
  breakCondition: row.theses?.[0]?.break_conditions?.[0]?.condition_text || '',
...
export async function listPositions() {
    ...
    .select(`
      *,
      ...
      reviews(
        ...
        would_buy_today, notes, created_at,
  ...
  return (data || []).map((row) => {
    if (row.theses?.[0]?.thesis_versions) {
      row.theses[0].thesis_versions.sort((a, b) => b.version_number - a.version_number);
    }
    if (row.reviews) {
      row.reviews.sort((a, b) => String(b.review_date).localeCompare(String(a.review_date)));
    }
...
export async function createPosition(userId, input) {
  const nextReviewDate = addDays(input.buyDate, input.reviewPlan);
  ...
  const { data: thesis, error: thesisError } = await supabase
    .from('theses')
    .insert({ user_id: userId, position_id: position.id })
    .select()
    .single();
  ...
  const { error: versionError } = await supabase.from('thesis_versions').insert({
    thesis_id: thesis.id,
    version_number: 1,
    thesis_text: input.thesis,
    expected_outcome: input.expected || null,
  });
  if (versionError) throw versionError;

  const { error: breakError } = await supabase.from('break_conditions').insert({
    thesis_id: thesis.id,
    condition_text: input.breakCondition,
  });
  ...
  const { error: decisionError } = await supabase.from('decisions').insert({
    ...
    decision: 'HOLD',
    rationale: 'Initial position recorded',
  });
...
export async function addThesisVersion(thesisId, currentVersion, thesisText, expectedOutcome, reasonForChange) {
...
export async function importLocalPositions(userId, localPositions) {
  const ids = [];
  for (const position of localPositions) {
    const id = await createPosition(userId, {
      ...
      reviewPlan: Number(position.reviewPlan || 30),
      thesis: position.thesis,
      expected: position.expected || '',
      target: position.target == null || position.target === '' ? null : Number(position.target),
      breakCondition: position.breakCondition,
    });
    ids.push(id);
  }
  return ids;
